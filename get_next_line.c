...
/*   Updated: 2024/12/10 19:22:49 by yuerliu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*letitgo(char *rdsponge, char **line)
{
	...
	endpoint = ft_strchr(rdsponge, '\n');
	if (!endpoint)
		endpoint = ft_strchr(rdsponge, '\0');
	l2b_trim = endpoint - rdsponge;
	*line = ft_substr(rdsponge, 0, l2b_trim + (endpoint && *endpoint == '\n'));
	if (!*line)
	{
		free(rdsponge);
		return (NULL);
